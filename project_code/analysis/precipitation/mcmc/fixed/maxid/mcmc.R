...
load(file.path(                                    # Load the mcmc initial values
  data.dir,
  stub,
  "initial_values",
  paste0("initial_values", last_it, ".Rdata")      # Initial values
))
...
runtime <- system.time(
  out <-
    mcmc_fixed_basis(
      ...
      df,                   # Data frame with spatial covariates (if used)
...
save(it, n, nloc, L, alpha, theta, tau, mar_gp_par, df, mar_gp_which,
     ...
     file = file.path(data.dir,stub,"initial_values",
                      paste0("initial_values",it,".Rdata"))
)
# Manage job list
