...
alpha <- 0.1                                                 # Positive stable parameter
...
mar_betas <- list(loc = 0, scale = 0, shape = 0)             # Linear coef for GEV GP means                                                
n <- nrow(y)
nloc <- ncol(y)                                              # number of spatial locations
L <- 15                                                      # number of random effects per year
zall <- rstabmix(                                            # Sample a process for init of lK and lB terms
  n,
  obs_coords,
  nbasis = L,
  alpha = alpha,
...
scale_form = ~ 1                                             # R formula for GEV log scale GP mean
...
mu <- sample_marpar_gp(loc_form,
                       ...
                       n,
                       obs_coords,
                       list(gvar = mar_gp_par[["mu"]][["gvar"]], gscl = mar_gp_par[["mu"]][["gscl"]]))
# GEV scale
sigma <- sample_marpar_gp(
  ...
  n,
  obs_coords,
  list(gvar = mar_gp_par[["sigma"]][["gvar"]], gscl = mar_gp_par[["sigma"]][["gscl"]]),
  log.link = TRUE
)
...
# List of initial tuning variances
tune_var <-
  list(
    alpha = 0.1,
    theta = 0.1,
